...
The existing `accountNumber` field in accounts is reused for OFX matching. No schema changes
required.
...
Import session state is **not persisted to CRDT** - it exists only during the import process in
React state.
...
Existing templates without these fields will use default values (schema's defaultValue mechanism
handles this).
