...
- **Current package:** **P20A** (HS-016 — truthful marketing pages) `implementing` rev 01 at BASE
  `42900590302030f9b38a56a560e9f53ecb3f65ed`. `p20a-implementer-01` dispatched (fresh, no-checkout)
  to inventory every public claim, remove budgeting/insight copy, and rewrite the landing surface to
  the frozen positioning — private categorising+allocating (not budgeting); CSV+OFX import;
  real-time collaboration; intelligent tags/aliases/allocations to new imports — with a
  claim-to-evidence table, component/accessibility tests, and a public landing-to-create E2E. HS-016
  is the SOLE package, so its scratch marker (`:328`) applies on P20A PASS. TWENTY of twenty-two
  requirements `passed` (HS-016, HS-021 remain).
- **Next action:** await `p20a-implementer-01` handback; verify-not-trust against git (delta
  confined to `src/app/(marketing)/**` + `src/components/features/landing/**` + tests; FS-001
  `settlement.ts` byte-identical; nothing under `src/lib/**`; no `as`/`any`/`!`; no secrets;
  canary==1), then dispatch a DISTINCT fresh reviewer. After P20A integration (marker at HS-016
  `:328`): P20B (HS-021) full cycle, then the P21 executable final audit (the completion gate).
...
- **Last ledger update:** 2026-07-27; **P20A / 01 DISPATCHED (HS-016 — truthful marketing pages)**.
  Pre-dispatch frozen-source integrity re-run and PASSED at the P20A boundary: actual
  `sha256sum specs/human-scratch.md` == rolling
  `9fcdc51e0d45176f887383bfdc9406ff19caecf6d9dc885e8f4b78219d2761cb` (actual==rolling; no marker
  this turn — HS-016's single-package marker applies only on P20A PASS); 008 canonical
  `0d0e2a141249ecace04b02b4cecbadb25ac5747faa24d59ab297aca509dcfe8c`, 715 lines, 25,441 bytes
  byte-exact; canary==1. P20A row -> `implementing` rev 01 at BASE `4290059`, evidence
  `evidence/P20A/implementation-01.md`. HANDOFF rewritten as the P20A IMPLEMENT dispatch (frozen
  text `:328-331`; editable scope = `src/app/(marketing)/**` +
  `src/components/features/landing/**` + tests; remove "Smart Budgeting"/"Spending
  Insights"/budgeting-tracking copy; claim-to-evidence table before copy; truthful privacy wording
  per threat model, no false absolutes; no feature advertised before usable; component/accessibility
  tests + public landing-to-create E2E; no `as`/`any`/`!`; secret-safety; FS-001 `settlement.ts`
  `010f3c93…` byte-identical). A fresh `p20a-implementer-01` is dispatched on the no-checkout
  discipline (commit forward on `4290059`). Rolling scratch SHA `9fcdc51e…` unchanged; TWENTY of
  twenty-two requirements `passed`; remaining packages P20A, P20B, P21.
...
| P20A    | HS-016         | Truthful marketing copy and responsive feature presentation                         | P17D, P19            | implementing | 01  | `42900590302030f9b38a56a560e9f53ecb3f65ed..` (dispatched)                            | `evidence/P20A/implementation-01.md` | —                           | —                                                                              |
...
HS-003 completed at Commit B `4290059` (linear single-parent). Serial loop advanced: with P10 passed the ready dispatchable leaf is **P20A** (HS-016; deps P17D, P19 both `passed`). **Frozen-source integrity re-run and PASSED at the P20A boundary:** actual `sha256sum specs/human-scratch.md` == rolling `9fcdc51e0d45176f887383bfdc9406ff19caecf6d9dc885e8f4b78219d2761cb` (actual==rolling; no marker this turn — HS-016 is a single-package requirement, its forward marker applies only on P20A PASS); 008 canonical `0d0e2a141249ecace04b02b4cecbadb25ac5747faa24d59ab297aca509dcfe8c`, 715 lines, 25,441 bytes byte-exact; canary==1. Frozen text `specs/human-scratch.md:328-331` (HS-016): update the marketing pages truthfully — private; categorising and allocating transactions, NOT budgeting; imports CSV and OFX; multiple people collaborate in real-time; intelligently applies tags/aliases/allocations to new imports; clear, succinct, not "markety".

Row P20A -> `implementing` rev 01 at BASE `42900590302030f9b38a56a560e9f53ecb3f65ed`, evidence `evidence/P20A/implementation-01.md`. HANDOFF rewritten as the P20A IMPLEMENT dispatch: editable product scope confined to `src/app/(marketing)/**` + `src/components/features/landing/**` (+ tests); audit and remove the current off-positioning copy (`FeaturesSection.tsx:68` "Smart Budgeting", `:76` "Spending Insights"; `CTASection.tsx:35` / `Footer.tsx:56` / `HeroSection.tsx:54` "track/tracking" language); produce a claim-to-evidence table BEFORE copy changes (no claim without a shipped/usable feature); rewrite to the frozen positioning; privacy wording must match the threat model with no false absolutes; responsive/dark/reduced-motion/accessibility intact; component/accessibility tests (guard against false budgeting headings) + a public landing-to-create E2E. Hard rules restated: no new `as`/`any`/`!` in product; secret-safety; FS-001 `src/lib/domain/settlement.ts` blob `010f3c93582a2ce311594d4dde8464760ca49c43` byte-identical and nothing under `src/lib/**` touched. A fresh `p20a-implementer-01` is dispatched on the no-checkout discipline (commit forward on `4290059`). Rolling scratch SHA `9fcdc51e…` unchanged; twenty requirements `passed`; remaining packages P20A, P20B, P21. Next: verify-not-trust the handback against git, then dispatch a DISTINCT fresh reviewer; on PASS this SOLE HS-016 package -> NON-markerless final integration applies the HS-016 scratch marker at `:328`.
