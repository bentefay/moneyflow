...
- **Current package:** P17D / 01 (HS-007 FINAL package — tag rule add/set parity + person-percentage (allocation) rule parity spanning all columns + apply-mode SELECT persistence (Q-P17B-03) + manual-row applicability + bulk/large-import/performance and every UX state) **implementing** (BASE `27ac503`). Reuses the P17A engine (already models `descriptionAlias`/`tags`/`allocation` + `TagRuleMode` and applies all three) + P17B shared `FieldRuleEditor`/CRUD/apply hooks + P17C inline robot/popup workflow — all call-not-edit. Additive-allowed for P17D (its committed scope, must stay additive and break no passed test): `preferences.ts` (apply-mode field), `schema.ts` (ONLY a new optional `lastApplyMode` slot on `userAutomationPreferenceSchema`), `field-rule-mutations.ts` (persist it), shared editor + `rule-editor-data.ts` (tag select + allocation columns + apply-mode wiring), transactions-feature files, tests. HARD byte-identical: settlement `010f3c93…` (FS-001), P16C `mutations.ts` `118e994a…` (allocations ONLY through `replaceTransactionAllocations`, never a direct allocation-map write; effective values FS-001-owned), engine `field-rules.ts` `4656c3c…`/import-commit/automation `{rules,apply,migration}` reused byte-identical (any needed change surfaced to root), both frozen specs, realtime, `supabase/migrations/**`. Fresh `p17d-implementer-01`, no-checkout discipline (commit on `27ac503`). Rolling scratch SHA `df8ad9ce…` unchanged; eighteen requirements `passed`; twenty-one of twenty-two packages `passed`.
- **Next action:** on `p17d-implementer-01` handback, root verify-not-trust against git (delta all in allowed paths; FS-001 settlement + P16C allocation-write boundary byte-identical; engine reused byte-identical or additive-change-surfaced; apply-mode chain genuinely additive with absent-key default; NO direct allocation-map write; tags add/set + allocation spanning set + manual-row applicability faithful to frozen `:288-295`; apply-mode SELECT persisted per `:270`; no `as`/`any`/`!`; no secrets; canary 1; rolling scratch SHA unchanged), then flip P17D -> `ready_for_review` and dispatch a DISTINCT fresh `p17d-reviewer-01` (never the implementer) over `27ac503..HEAD`. On review PASS: this is the LAST HS-007 package, so integration is NON-markerless — Commit A persists review+evidence+PASS event, Commit B flips P17D -> `passed` AND, HS-007 now having all four packages passed + Q-P17B-03 delivered, applies the HS-007 scratch marker (`- [] ` -> `- [x] ` on the frozen block first line), advances the rolling scratch SHA, and flips the HS-007 requirement row -> `passed`, with the normalized-block/rolling-SHA check re-run green. Then serial loop -> P10 (HS-003; deps P05,P08 passed), the last leaf before P20A/P20B/P21. PROCESS serial (one package `implementing`), so only P17D is active now.
...
- **Last ledger update:** 2026-07-27; **P16E / 02 handback verified; dispatched distinct reviewer**.
  `p16e-implementer-02` returned `ready_for_review` at HEAD `bb12e0c` (F-1 fix). Root independently verified:
  rev-02 delta = EXACTLY the 3 allowed paths; EMPTY boundaries (`src/lib/**`, P16D grid byte-identical,
  migrations, realtime, `vault_ops`); F-1 root cause removed (`selectedTransactionIds` param-free BASE form;
  one-shot `landedSourceId` + `router.replace`); single engine + no cache; two new F-1 E2E regressions; no
  premature review file. P16E `implementing -> reviewing`, rev 02. DISTINCT reviewer `p16e-reviewer-02`
  dispatched over `191d070..bb12e0c`. Rolling scratch SHA unchanged `df8ad9ce…`; seventeen requirements
  `passed`; FS-001 immutable/open; no marker
...
| P17D    | HS-007         | Tags/allocation parity, bulk/new application, performance and polish                | P17C                 | reviewing | 01  | `27ac503..aad518e`                                                                      | `evidence/P17D/implementation-01.md` | `reviews/P17D-review-01.md` | —                                          |
