...
- **Current package:** **P10 / 01 `reviewing`** (HS-003 Loro ephemeral presence + active transaction). Implement handback verified by root against git: linear single-parent chain `54a88ae->f6ae3fe->d832443->71c378c`; delta entirely in sync/crypto/presence/hook/transaction-UI + test paths (no `src/lib/crdt/**`, no root-owned files, no migrations); FS-001 `settlement.ts` + P05 `rls-audit.sql` byte-identical; product code cast-free; no secrets; canary==1; scratch SHA unchanged (`1b56b21c…`; P10 marker applies only on PASS). Root spot-check typecheck clean. Dispatching a DISTINCT fresh reviewer. Nineteen of twenty-two requirements `passed`.
- **Next action:** await the P10 reviewer VERDICT. On PASS: two-commit integration (Commit A persist review+evidence, row -> reviewing; Commit B flip row -> `passed` + apply the HS-003 authorized forward marker at scratch `:161`, advance rolling SHA, add HS-003 to authorized checked IDs, flip HS-003 requirement -> `passed`). Then P20A (HS-016), P20B (HS-021), P21 final audit.
...
- **Rolling scratch SHA-256:** `1b56b21c0787b4fdebc800d1256816244e5b907549fffb2b067c9295c9b85a57`
- **Authorized checked HS IDs:** HS-001, HS-002, HS-004, HS-005, HS-006, HS-007, HS-008, HS-009,
  HS-010,
...
- **Last ledger update:** 2026-07-27; **P10 / 01 implement handback VERIFIED -> `reviewing`; DISTINCT reviewer
  dispatched**. `p10-implementer-01` delivered at HEAD `71c378c` (linear chain `54a88ae->f6ae3fe->d832443->71c378c`).
  Root verify-not-trust PASSED: 17 product/test files + evidence, ALL in allowed sync/crypto/presence/hook/UI/test
  paths; NO `src/lib/crdt/**`, NO root-owned/marker/spec file, NO migration; FS-001 `settlement.ts`
  (`010f3c93…`) and P05 `rls-audit.sql` (`9b04bef7…`) byte-identical BASE->HEAD; product code cast-free (only an
  ESM re-export alias + one test-fixture `as unknown as never` flagged for the reviewer, non-blocking per
  precedent); no secret/presence-key/vault-plaintext; canary==1; scratch SHA unchanged `1b56b21c…` (P10 marker
  applies only on PASS). Root spot-check typecheck clean. Q-P10-01 (abrupt-close expiry backstop) + Q-P10-02
  (cell-level editing-field granularity) recorded with reversible defaults in place. Nineteen of twenty-two
  requirements `passed`; HS-003, HS-016, HS-021 remain; no marker event or rollback batch active
...
| P10     | HS-003         | Encrypted Loro EphemeralStore presence and active transaction                       | P05, P08             | reviewing | 01  | `54a88ae..71c378c`                                                                    | `evidence/P10/implementation-01.md` | —                          | —                                          |
