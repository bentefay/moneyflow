...
- **Current package:** P17A / 01 (HS-007 automation engine) **implementing**. Serial dispatch after FS-001 completed at `a09c4b4`. P17A is the model + deterministic engine slice of HS-007 (`specs/human-scratch.md:248-295`, packages P17A-D): typed field-specific exact-description rule keys with uniqueness; optional amount/account constraints and deterministic unscoped/amount/account/account+amount precedence; per-user-per-vault preferences; date/import boundary semantics for new/newer; safe migration of existing generic rules; highest-rule application at import + explicit bulk ops (description ignores manual rows, tag/allocation honor them). Allocation rule values are COMPLETE EXPLICIT sets written ONLY through P16C's validated atomic complete-set API — never clamped/normalized, never bypassing validation via import/apply/undo/migration; no duplicated settlement/remainder logic. BASE `a09c4b4`; deps P11C/P14/P16E (+P02/P09) all `passed`. Frozen-source integrity re-verified: scratch whole-file SHA == rolling `df8ad9ce…` (PROCESS §242, the file legitimately diverges from its original identity `b91ca932…` only via 17 authorized markers — NOT drift), 008 canonical `0d0e2a14…`/715/25,441 byte-exact. Engine-only: NO UI (P17B/C/D own `components/features/automations/**` + `AutomationDropdown.tsx`); settlement engine `src/lib/domain/settlement.ts` and both frozen sources stay byte-unchanged; `supabase/migrations/**` expected EMPTY (rules live in the encrypted vault/CRDT, migration is in-code). Fresh implementer `p17a-implementer-01` to be dispatched. Rolling scratch SHA `df8ad9ce…` unchanged; eighteen requirements `passed`.
- **Next action:** rewrite HANDOFF for P17A/01 IMPLEMENT, dispatch `p17a-implementer-01`, arm implementer-liveness cron. On handback: verify-not-trust (delta within allowed paths, boundaries EMPTY, settlement byte-identical, no P16C-API bypass, tests present), set `ready_for_review`, then dispatch a DISTINCT `p17a-reviewer-01`. HS-007 stays unchecked until all four packages P17A-D pass.
...
| P17A    | HS-007         | Automation schema/migration, exact matcher, precedence, preferences, import engine  | P11C, P14, P16E      | implementing | 01  | `a09c4b4..pending`                                                                   | `evidence/P17A/implementation-01.md` | —                          | —                                          |
