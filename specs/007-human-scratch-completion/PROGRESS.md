...
- **Current package:** P17C / 01 (HS-007 — description inline workflow: contextual popup reusing the P17B `FieldRuleEditor`, robot normal/red-drift/hidden state per row, inline alias-edit -> UPDATE rule, apply-this/all/new scoped to the row date) **passed** (integration `ea2ad75`; control flip this commit). BASE `0d3de91`; deps P17B (passed). Reuses P17A matchers (`selectWinningRule`/`ruleMatchesSubject`/`ruleScopeRank`/`isNewerTransactionDate` from `domain/automation/rules.ts`) and P17B editor/CRUD/apply hooks; adds one thin ADDITIVE single-transaction-apply vault action (new `src/lib/crdt/**` file) routing through P17A `applyFieldRulesToTransaction` + P16C only. Engine/import/settlement/P16C/schema/defaults byte-IDENTICAL; allocations P16C-only; apply-mode SELECT persistence stays Q-P17B-03/P17D (schema boundary). P17C implementer clears the stray `locationOf` lint warning. Fresh `p17c-implementer-01`, no-checkout discipline. Rolling scratch SHA `df8ad9ce…` unchanged; eighteen requirements `passed`; twenty of twenty-two packages `passed`. BASE current HEAD `5e2ddd0`; deps P17A (passed) + P02 (passed). P17B delivers ONE accessible shared rule editor that powers the Automations page now (and the P17C contextual popup later): field selector, optional amount/account constraints, the four modes, values, validation, delete, apply-all and apply-new — reworking the existing pre-redesign automations UI (`ActionEditor`/`ConditionEditor`/`AutomationRow`/`AutomationsTable`/`page.tsx`) onto the P17A field-rule model. P17A exposed NO rule-CRUD/preference WRITE actions (it owns model/read/apply/migration/pure-preference-transforms only), so P17B additively adds the thin field-rule create/update/delete + remembered-preference persistence vault actions in NEW/additive `src/lib/crdt/**` files that REUSE P17A's `fieldRuleSchema`/validation and `nextUserPreference` — this completes the editor's committed scope (more work, not a reduction; P17A stays legitimately complete and byte-untouched). Engine/apply/migration/import-commit (`field-rules.ts`, `import-commit.ts`, automation domain), settlement, and P16C `replaceTransactionAllocations` stay byte-IDENTICAL; allocations only ever through P16C. Fresh `p17b-implementer-01` on the no-checkout discipline. Rolling scratch SHA `df8ad9ce…` unchanged; eighteen requirements `passed`; nineteen of twenty-two packages `passed`.
- **Next action:** serial loop advances. P17C `passed` (integration Commit A `ea2ad75`; Commit B control flip this commit). Dispatchable set recomputed = {P17D (dep P17C passed), P10 (deps P05,P08 passed)}; every other queued package remains dependency-blocked. Choose **P17D** next — the FINAL HS-007 package (tags/allocation parity, bulk/large-import polish, performance) and the last gate before HS-007 can be checked; it also OWNS the tracked Q-P17B-03 apply-mode SELECT persistence (additive `applyMode` slot in `userAutomationPreferences` + editor wiring). P10 (HS-003, a P21 pre-req leaf) follows. PROCESS is serial (one package `implementing`, one HANDOFF), so only P17D is dispatched. HANDOFF will be rewritten as the P17D IMPLEMENT dispatch at BASE = the control HEAD produced by this Commit B (resolved at P17D dispatch); fresh `p17d-implementer-01` on the no-checkout discipline. Frozen-source integrity (scratch==`df8ad9ce…`; 008 canonical `0d0e2a14…`/715/25,441) re-run at the P17D boundary before dispatch. HS-007 stays UNCHECKED until P17D passes AND Q-P17B-03 is delivered; TWENTY-ONE of twenty-two packages now `passed`.
