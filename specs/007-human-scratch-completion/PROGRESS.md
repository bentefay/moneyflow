...
- **Current package:** **P10 / 01 `implementing`** (HS-003 — replace the misnamed Supabase-Presence system with a real encrypted Loro `EphemeralStore` protocol for vault presence + per-session identity + active transaction + editing field; no ephemeral value in durable CRDT/IndexedDB/server; authenticated encrypted payloads only over the P05-authorized channel). BASE = `41c6c81` (Commit B HEAD). Deps P05, P08 passed. Dispatched to a fresh implementer on no-checkout discipline. Nineteen of twenty-two requirements `passed` (HS-003, HS-016, HS-021 remain).
- **Next action:** await the P10 implement handback; verify-not-trust against git (delta in allowed sync/crypto/CRDT/presence/UI paths, frozen blobs byte-identical, no `as`/`any`/`!`, no secrets/vault-plaintext/presence-key material in code/logs/fixtures/evidence, canary==1, linear chain), then dispatch a DISTINCT fresh reviewer. After P10 -> P20A (HS-016; deps P17D, P19 passed), P20B (HS-021), P21 final audit.
...
- **Last ledger update:** 2026-07-27; **P10 / 01 dispatched (HS-003 — Loro ephemeral presence + active
  transaction)**. HS-007 completed at Commit B `41c6c81` (P17D `passed`; forward marker applied, rolling scratch
  SHA `df8ad9ce… -> 1b56b21c…`, 18 authorized checked IDs, normalized-block check 0 mismatches). Serial loop
  advanced: dispatchable leaf = P10 (deps P05, P08 both `passed`); frozen-source integrity re-run and PASSED at the
  P10 boundary (scratch `sha256sum` == rolling `1b56b21c…`; 008 canonical `0d0e2a14…`, 715 lines, 25,441 bytes
  byte-exact). Row P10 -> `implementing` rev 01 at BASE `41c6c81`; HANDOFF rewritten as the P10 IMPLEMENT dispatch.
  Nineteen of twenty-two requirements `passed`; HS-003, HS-016, HS-021 remain; no marker event or rollback batch active
...
| P10     | HS-003         | Encrypted Loro EphemeralStore presence and active transaction                       | P05, P08             | implementing | 01  | `41c6c81..HEAD`                                                                       | `evidence/P10/implementation-01.md` | —                          | —                                          |
