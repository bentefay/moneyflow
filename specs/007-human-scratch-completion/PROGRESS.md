...
- **Current package:** P17A / 01 (HS-007 automation engine) **implementing** — production-import wiring in progress. Independent opus-tier adjudicator RULED **IN_P17A** (high confidence) on Q-038: applying rules at the real import event is P17A committed scope, not a later package. Basis it cited: frozen `human-scratch.md:~272,~287` (a rule 'will run for newly imported transactions' / rules 'apply to new imports' = the import event, not a dormant fn); P17A task acceptance 'apply the highest rule at import' + required 'Integration for import application' test; the P16C-at-import allocation constraint presupposes P17A applies at import; PROGRESS P17A row = 'import engine'; P17B/C/D carve out non-overlapping UI/bulk concerns. It also ruled resolving the `ApplicationVaultState = Omit<VaultState,'descriptionAliases'>` alias barrier is inside P17A (route the description-alias write through the existing P11 path from the import seam, or a minimal additive context seam). So this is scope COMPLETION (more work), NOT a reduction — no further adjudication. Dispatching continuation `p17a-implementer-01c` (paths widened by the ONE minimal call-site `src/app/(app)/imports/new/page.tsx` + additive `src/lib/crdt/**` incl. context.tsx-if-unavoidable; NO automation UI) to invoke `applyFieldRulesToImport` at `createImportBatch`, resolve the alias-write barrier via the P11 path, keep allocations on P16C, with an E2E proving a real import applies rules. Rev stays 01 (no review yet). Rolling scratch SHA `df8ad9ce…` unchanged; eighteen requirements `passed`.
- **Next action:** dispatch `p17a-implementer-01c` for the production-import wiring (Q-038 IN_P17A). On handback: verify-not-trust (delta within widened allowed paths, settlement + P16C `mutations.ts` byte-identical, automation-UI/realtime/migrations/frozen boundaries byte-empty, description-alias write reuses P11 not a cast, allocations only via P16C, no secrets), re-run gates independently, then set `ready_for_review` and dispatch a DISTINCT `p17a-reviewer-01` over `a09c4b4..<final HEAD>` (root ledger commits interleaved — reviewer ignores them). HS-007 stays unchecked until all four packages P17A-D pass.
...
| P17A    | HS-007         | Automation schema/migration, exact matcher, precedence, preferences, import engine  | P11C, P14, P16E      | implementing | 01  | `a09c4b4..21507dc`                                                                   | `evidence/P17A/implementation-01.md` | —                          | —                                          |
