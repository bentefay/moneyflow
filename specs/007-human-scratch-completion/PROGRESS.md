...
- **Next action:** **P20B (HS-021) DISPATCHED** to `p20b-implementer-01` over BASE `659ca20`
  (boundary integrity GREEN: scratch == rolling `f46c2d35…`, 008 `0d0e2a14…`, canary==1). Await the
  implement handback (full-codebase style-guide sweep: bounded inventory by guide×subsystem, fix
  concrete violations, no aesthetic churn, frozen sources + `settlement.ts` byte-identical, no new
  casts, regression tests + E2E flake sample). Then verify-not-trust against git and dispatch a
  DISTINCT `p20b-reviewer-01`. HS-021 is a single-package requirement, so on PASS the marker at
  scratch `:159` applies on P20B integration — **apply it via `sed`, NEVER Edit/Write** (formatter
  reflow hazard; see the P20A/02 Commit-B event and memory). Then the **P21 executable final audit**
  (`tasks/P21-final-audit.md`, the completion gate) -> FINAL-AUDIT.md. Deferred sweeps the reviewer
  should confirm addressed-or-deferred: Q-P17D-02 (dead `description-rule-state.ts` + stale
...
| P20B    | HS-021         | Full-codebase style-guide/code-quality sweep after all feature work                 | P20A                 | implementing | 01  | `659ca20..` (rev 01 dispatched)                                                      | `evidence/P20B/implementation-01.md` | —                                         | —                                                                                          |
