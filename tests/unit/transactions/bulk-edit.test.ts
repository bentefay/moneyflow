...
describe("useBulkEdit", () => {
	...
	describe("setTags", () => {
		it("applies tags to all selected transactions", async () => {
			...
			await act(async () => {
				await result.current.setTags(["tx-1", "tx-2", "tx-3"], ["tag-a", "tag-b"], mutation);
			});
	...
	describe("progress tracking", () => {
		it("updates progress during operation", async () => {
			...
			act(() => {
				operationPromise = result.current.setTags(["tx-1", "tx-2", "tx-3"], ["tag-a"], mutation);
			});
	...
	describe("error handling", () => {
		...
		it("wraps non-Error objects in Error", async () => {
			...
			expect(props.onOperationError).toHaveBeenCalledWith("setTags", expect.any(Error));
			expect((props.onOperationError.mock.calls[0][1] as Error).message).toBe("string error");
		});
