...
describe("Automation Engine Integration", () => {
    ...
    describe("Complex condition patterns", () => {
        ...
        it("handles amount-based conditions", () => {
            ...
            expect(
                evaluateAutomations([largeExpenseAutomation], fixtures.transactions.groceries)
                    .matched
            ).toBe(true);
            // -850 doesn't match (only 3 digits)
            expect(
                evaluateAutomations([largeExpenseAutomation], fixtures.transactions.starbucks)
                    .matched
            ).toBe(false);
        });
    });
