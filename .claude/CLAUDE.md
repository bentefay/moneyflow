...
- CLAUDE.md must be updated alongside code changes to keep instructions current.
- .claude/skills/* must be created/updated as new folders/domains are added.
...
- **Feature Specs**: `specs/` - MVP specifications
- **Domain Guides**: `.claude/skills/` - Domain-specific instructions
...
Tests MUST be written alongside features. See Constitution VII for philosophy. Always load .claude/skills/e2e.md when writing e2e tests. Do NOT use playwright with the --debug flag, as it blocks forever and doesn't work for agents.
