...
export function VaultProvider({ children, registerDisconnect }: VaultProviderProps) {
    ...
    // Publish the durability barrier for the E2E harness; a no-op in production builds. Declared
    // ahead of the initialization effect below so a live manager always implies a live seam. The
    // harness requires the seam on every route this provider serves and fails loudly without it
    // (`tests/e2e/helpers/persistence.ts`), so deleting this line breaks E2E rather than silently
    // reopening the lost-write class it closes.
