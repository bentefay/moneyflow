...
export function PersonAllocationCell({
	...
	if (isEditing) {
		...
		return (
			<div ref={containerRef} className={cn("w-64", className)}>
				<div className="rounded-md border bg-background p-3 shadow-lg" onKeyDown={handleKeyDown}>
					<h4 className="mb-2 font-medium text-xs uppercase tracking-wide">Allocations</h4>
	...
	if (allocations.length === 0) {
		return (
			<div
				...
				className={cn(
					"cursor-pointer text-muted-foreground text-sm italic",
	...
	return (
		...
		>
			...
			{otherCount > 0 && <span className="ml-1 text-muted-foreground">+{otherCount}</span>}
		</div>
	);
}
