	...
	return (
		<div className={cn("space-y-4", className)}>
			...
			<div className="grid gap-4 rounded-lg bg-muted/50 p-4 sm:grid-cols-3">
				...
				<div className="text-center">
					<div className={cn("font-bold text-2xl", validCount > 0 && "text-green-600")}>
						{validCount}
					</div>
					<div className="text-muted-foreground text-sm">Valid Transactions</div>
				</div>
				<div className="text-center">
					<div className={cn("font-bold text-2xl", errorCount > 0 && "text-red-600")}>
						{errorCount}
					</div>
				...
				<table className="w-full text-sm">
					...
					<tbody className="divide-y">
						{displayedTransactions.map((tx) => (
							...
							>
								<td className="px-3 py-2 text-muted-foreground">{tx.rowIndex + 1}</td>
