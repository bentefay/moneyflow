...
export function FormattingTab({
    ...
    const currentNumberFormat = NUMBER_FORMAT_OPTIONS.find(
        (opt) =>
            opt.thousand === formatting.thousandSeparator &&
            opt.decimal === formatting.decimalSeparator
    );
                ...
                <div className="flex items-center space-x-3">
                    <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                            onFormattingChange({ hasHeaders: checked === true })
                        }
                    />
            ...
            {isCsv && (
                <div className="space-y-2">
                    ...
                    {sampleDates.length > 0 && (
                        <p className="text-muted-foreground text-xs">
                            Sample: <code className="bg-muted rounded px-1">{sampleDates[0]}</code>
                        </p>
                    )}
                </div>
            )}
            ...
            {isCsv && (
                <div className="space-y-2">
                    ...
                    >
                        ...
                        <SelectContent>
                            {NUMBER_FORMAT_OPTIONS.map((opt) => (
                                <SelectItem
                                    key={opt.label}
                                    value={`${opt.thousand}|${opt.decimal}`}
                                >
                                    {opt.label}
                                </SelectItem>
                            ))}
                    ...
                    {sampleAmounts.length > 0 && (
                        <p className="text-muted-foreground text-xs">
                            Sample:{" "}
                            <code className="bg-muted rounded px-1">{sampleAmounts[0]}</code>
                        </p>
                    )}
                </div>
            )}
